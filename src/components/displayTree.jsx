...
export default function TreePage() {
  ...
  useEffect(() => {
    ...
    // Handle the D3 rendering logic only if prerequisiteData exists
    const width = 928;
    const height = 900;
    ...
    node
      ...
      .attr("r", 35)
      .attr("fill", (d) => {
        if (locked && locked.includes(d.id)) return "gray";
        if (completed && completed.includes(d.id)) return "green";
        if (incomplete && incomplete.includes(d.id)) return "red";
        if (requirement_satisfied && requirement_satisfied.includes(d.id)) return "purple";
        return "red";
      });
    ...
    node.each(function (d) {
      if (locked && locked.includes(d.id)) {
        tippy(this, {
